...
// Only an uploaded logo (stored as a data: URL) is printed; the server never downloads a logo
// from a web address. With an external logo URL the ticket shows the brand name without an image.
const logoBuffer = (logoUrl) => {
  const candidate = typeof logoUrl === 'string' ? logoUrl.trim() : '';
  if (!candidate) return Buffer.from(DEFAULT_LOGO_SVG);
  if (candidate.startsWith('data:')) {
    const base64 = candidate.split(',')[1] || '';
    if (base64 && base64.length <= LOGO_MAX_BYTES * 2) return Buffer.from(base64, 'base64');
  }
  return null;
};
...
const buildEscPos = async ({ labels, brandLine, logoUrl, ticketNumber, serviceName, date, waitLine, qrUrl, footer }) => {
  ...
  const raster = await logoToRaster(logoBuffer(logoUrl));
