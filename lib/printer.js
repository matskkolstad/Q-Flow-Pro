...
const buildEscPos = async ({ labels, brandLine, logoUrl, ticketNumber, serviceName, date, waitLine }) => {
  const ESC = '\x1b';
  const GS = '\x1d';
  const reset = ESC + '@';
...
const buildEposXml = ({ labels, brandLine, ticketNumber, serviceName, date, waitLine }) => `
 ...
 */
export const printTicket = async ({ printer, ticket, serviceName, waitTime, language, brandText, brandLogoUrl, log = () => {} }) => {
  ...
  const content = { labels, brandLine, logoUrl: brandLogoUrl, ticketNumber: String(ticket.number), serviceName, date, waitLine };
